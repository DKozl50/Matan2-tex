...
\end{proof}
   ...
   \definition{
    ...
    \subsection*{102. Получите разложение для $\ln (1 + x)$ интегрированием разложения для $(1 + x)^{-1}.$ Каково мн-во сходимости ряда? На каком мн-ве сумма ряда представляет собой исходную функцию?}
    
    Рассмотрим $\displaystyle f(x) = (1+ x)^{-1} = \sum_{n = 0}^{\infty} (-x)^n$ - сходится при $|x| < 1.$ оответственно, радиус сх-ти $ = 1.$
    \begin{flalign}
    & \displaystyle \ln (1 + x) = \int_{0}^{x} \frac{1}{1 + t} dt = \int_{0}^{x} \left (\sum_{n = 0}^{\infty} (-t)^n \right) dt = \sum_{n = 0}^{\infty} \frac{(-1)^{n} \cdot x^{n + 1}}{n + 1}   \text{ при } |x| <  1.
    \end{flalign} 
    Заметим, что у нового ряда $R = 1$ (совпадает с  исходным). А при $x = 1$ ряд $\; \displaystyle \sum_{n = 0}^{\infty} \frac{(-1)^n}{n + 1} $ сходится. Следовательно, мн-во сходимости нового ряда будет $(-1; 1].$ 
